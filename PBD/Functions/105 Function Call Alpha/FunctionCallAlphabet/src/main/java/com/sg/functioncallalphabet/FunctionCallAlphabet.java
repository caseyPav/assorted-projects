...
public class FunctionCallAlphabet {
    ...
    public static void main( String[] args )
	{
		...
		f("foolish");// ???? f( ???? );    // displays the word you pass it beginning with "F"
		g();// ???? g( ???? );    // returns the word to be displayed
		h(1);// ???? h( ???? );    // tell it how many times to display the word (1)
		System.out.println();

		i(31);// ???? i( ???? );    // pass it any integer and it will display a word beginning with I 
		j(3);// ???? j( ???? );    // returns a different word depending on what you pass it (1-3)
		k();// ???? k( ???? );    // displays a word beginning with K
		l(true,true);// ???? l( ???? );    // displays a different word depending on the two booleans you pass it
		m(true,true);// ???? m( ???? );    // displays a different word depending on the two booleans you pass it
		n("noon");// ???? n( ???? );    // displays the word you pass it beginning with "N"
		...
		t("tuba");// ???? t( ???? );    // displays the word you pass it beginning with "T"
		u();// ???? u( ???? );    // returns the word to be displayed
		v(1);// ???? v( ???? );    // tell it how many times to display the word (1)
		w(1);// ???? w( ???? );    // pass it any integer and it will display a word beginning with W 
		x(2);// ???? x( ???? );    // returns a different word depending on what you pass it (1-2)
		y();// ???? y( ???? );    // displays a word beginning with Y
		z(false, false);// ???? z( ???? );    // returns a different word depending on which two boolean values you pass it
		System.out.println();
	}
